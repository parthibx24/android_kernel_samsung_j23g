...
static void sm5504_regs_init(sm5504_chip_t *chip);
#define I2C_RW_RETRY_MAX    3
#define I2C_RW_RETRY_DELAY  10
static int retry_sm5504_reg_write(struct sm5504_chip *chip, int reg, int value)
{
	int result = -1, i;
	for (i = 0; i < I2C_RW_RETRY_MAX && (result < 0); i++) {
		result = i2c_smbus_write_byte_data(chip->iic, reg, value);
        pr_info("%s: write : result = 0x%x  retry = %d (reg = 0x%x, value = 0x%x)\n",__FUNCTION__, result, i, reg, value);
		if (result < 0)
			msleep(I2C_RW_RETRY_DELAY);
	}
	return result;
}
static int retry_sm5504_reg_read(struct sm5504_chip *chip, int reg)
{
	int result = -1, i;
	for (i = 0; i < I2C_RW_RETRY_MAX && (result < 0); i++) {
		result = i2c_smbus_read_byte_data(chip->iic, reg);
        pr_info("%s: read : result = 0x%x  retry = %d (reg = 0x%x) \n",__FUNCTION__, result, i, reg);
		if (result < 0)
			msleep(I2C_RW_RETRY_DELAY);
	}
	return result;
...
{
	int ret;
    int intmask1, intmask2;
    
	ret = retry_sm5504_reg_read(chip, SM5504_REG_CONTROL);
	if (ret < 0)
	{
		RTERR("%s line %d : return %d\n",__FUNCTION__, __LINE__,ret);
	}
    pr_info("%s : CONTROL = 0x%x\n",__FUNCTION__, ret);
    if ((ret != 0xa0) && ((ret == 0xE5) || ((ret&0xE0) != 0xE0) || ((ret&0x01) == 0x01 ) || ( (ret&0x08) == 0x08 ) || ( (ret&0x10) == 0x10 )))
	{
        // register init
        sm5504_regs_init(chip);
    }
	intmask1 = retry_sm5504_reg_read(chip, SM5504_REG_INTERRUPT_MASK1); 
	intmask2 = retry_sm5504_reg_read(chip, SM5504_REG_INTERRUPT_MASK2); 
    pr_info("%s : intmask1 = 0x%x , intmask2 = 0x%x\n",__FUNCTION__, intmask1, intmask2);
    if ( (intmask1 != 0x20) || (intmask2 != 0x24) ) 
    {
        // register init
        sm5504_regs_init(chip);
    }
...
static void sm5504_recovery_work(struct work_struct *work)
{
	int ret;
	sm5504_chip_t *chip = container_of(to_delayed_work(work),
					   sm5504_chip_t, recovery_work);
...
static void sm5504_init_work(struct work_struct *work)
{
	...
	sm5504_reg_read(chip, SM5504_REG_INT_FLAG1);
	 sm5504_reg_read(chip, SM5504_REG_INT_FLAG2);  // The interrupt register should be read  both INT1 and INT2.
	//sm5504_clr_bits(chip, SM5504_REG_CONTROL, 0x60);  // The unnecessary code. This code is needed to RT8973 chip of richtek.
	//msleep_interruptible(1);                           // The unnecessary code. This code is needed to RT8973 chip of richtek.
	//sm5504_set_bits(chip, SM5504_REG_CONTROL, 0x60);    // The unnecessary code. This code is needed to RT8973 chip of richtek.
	/* enable interrupt */
...
static void sm5504_init_regs(sm5504_chip_t *chip)
{
	...
	queue_delayed_work(chip->wq, &chip->init_work, msecs_to_jiffies(1000));
}
...
static void sm5504_regs_init(sm5504_chip_t *chip)
{
	...
	sm5504_reg_write(chip, SM5504_REG_INTERRUPT_MASK2, 0x24);
    sm5504_reg_write(chip, 0x20,0x06);
    sm5504_reg_write(chip, SM5504_REG_CONTROL, 0xE4);
    return;
}
#ifdef CONFIG_MUIC_FACTORY_EVENT
...
//#endif
	...
	INIT_DELAYED_WORK(&chip->recovery_work, sm5504_recovery_work);
...
//#endif
	...
	queue_delayed_work(chip->wq, &chip->recovery_work, msecs_to_jiffies(10000));
#ifdef CONFIG_MUIC_FACTORY_EVENT
