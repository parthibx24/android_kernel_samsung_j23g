...
#define USE_OPEN_CLOSE	0
#define USE_PIN_CTRL	0
...
void tc300k_power(struct tc300k_data *data, int onoff)
{
	...
	tk_vdd = regulator_get(&client->dev, "tk_vdd");

	if (IS_ERR(tk_vdd)) {
		dev_err(&client->dev, "TK regulator_get error!\n");
		return;
	}

	if (onoff) {
		if (!regulator_is_enabled(tk_vdd)) {
			ret = regulator_enable(tk_vdd);
			if (ret) {
				dev_dbg(&client->dev, "power on error!!\n");
				return;
			}
			dev_info(&client->dev, "TK power on!\n");
		} else
			dev_dbg(&client->dev, "already power on!\n");
	} else {
		if (regulator_is_enabled(tk_vdd)) {
			ret = regulator_disable(tk_vdd);
			if (ret) {
				dev_dbg(&client->dev, "power off error!\n");
				return;
			}
			dev_info(&client->dev, "TK power off!\n");
		} else
			dev_dbg(&client->dev, "already power off!\n");
	}

	regulator_put(tk_vdd);
...
static int tc300k_write_fw(struct tc300k_data *data)
{
	...
	dev_info(&client->dev, "fw write code size = %#x (%u)",
