...
static int gen_panel_set_acl(struct lcd *lcd)
{
	if (lcd->hbm)
		return 0;

	if (lcd->acl) {
		pr_info("[LCD] %s, set acl on (acl %d, hbm %d)\n",
				__func__, lcd->acl, lcd->hbm);
		gen_panel_write_op_cmds(lcd, PANEL_ACL_ON_CMD);
	} else {
		pr_info("[LCD] %s, set acl off\n", __func__);
...
static int gen_panel_set_brightness(void *prvinfo, int intensity)
{
...
#ifdef CONFIG_GEN_PANEL_DYNAMIC_AID
	if (!is_sdimming_init) {
		pr_warn("[LCD] %s, gen_panel_dimming_init\n", __func__);
		ret = gen_panel_dimming_init(lcd);
		if (ret < 0) {
			mutex_unlock(&lcd->access_ok);
			return 0;
		}
	...
	if (IS_HBM(bl_info->auto_brightness)) {
		...
		gen_panel_set_acl(lcd);
	}
	gen_panel_write_op_cmds(lcd, PANEL_GAMMA_UPDATE_CMD);
	gen_panel_write_op_cmds(lcd, PANEL_NV_DISABLE_CMD);
	if (!lcd->hbm)
			pr_info("%s, intensity %d, candela %d, index %d\n",
			__func__, intensity, candela_table[index], index);
#else
...
size_t gen_panel_read_hbm(struct lcd *lcd)
{
	...
	if (!op_cmd_valid(lcd, PANEL_HBM_ON))
		return -EINVAL;
/*
	cmd = find_op_cmd(lcd, PANEL_HBM_ON);
	memcpy(&cmd->desc[4].data[data_offset], &reg_B5h[18], 1);
	print_cmd_desc(&cmd->desc[4]);
*/
	desc = find_op_cmd_desc(lcd, PANEL_HBM_OFF, 0xB6);
	if (!desc) {
		pr_err("%s: elvss hbm off not found\n", __func__);
		return -EINVAL;
	}
	memcpy(&desc->data[data_offset], &reg_B6h[16], 1);
	print_cmd_desc(desc);
		...
		struct device_node *np, struct lcd *lcd)
{
	...
	lcd->id = panel_ids[panel_id_index];
